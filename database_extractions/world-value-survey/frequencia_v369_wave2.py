"""
Script para calcular a frequência da variável V369 da wave 2 do World Values Survey.
...
# Encontrar o índice da coluna V369
def find_v369_index(header_row):
    """Encontra o índice da coluna V369 no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v369_index = find_v369_index(header)
    
    if v369_index is None:
        print("ERRO: Coluna V369 não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V369 encontrada no índice: {v369_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v369_index:
            value = row[v369_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v369_values.append(float(value))
                else:
                    v369_values.append(None)
            except ValueError:
                v369_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V369 (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v369_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V369 (Wave 2 - 1991)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v369_values)
...
numeric_values = [v for v in v369_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
