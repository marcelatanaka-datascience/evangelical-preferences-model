"""
Script para testar o modelo com interações entre variáveis religiosas e religião
(evangélico vs não-evangélico) segundo a teoria proposta na tese.

Modelo teórico:
Yi = β₀ + (ModeloBase) + β₁(Freqi × GrupoEvi) + β₂(Perti × GrupoEvi) + 
     β₃(Importi × GrupoEvi) + β₄(Identi × GrupoEvi) + γXi + εi

onde:
- Yi: preferência política (y_moral_inv ou y_intitucional_4vars)
- ModeloBase inclui:
  - Freqi: frequência a cultos (freq_inv_z)
  ...
  - Xi: controles sociodemográficos (sexo, raça, escolaridade, ano/coorte)
- Interações:
  - Freqi × GrupoEvi
  - Perti × GrupoEvi
  - Importi × GrupoEvi
...
MAPEAMENTO_NOMES = {
    ...
    'GrupoEv': 'Evangélico',
    'freq_inv_z_x_GrupoEv': 'Frequência a serviços religiosos × Evangélico',
    'pert_r_z_x_GrupoEv': 'Pertencimento × Evangélico',
    'import_relig_z_x_GrupoEv': 'Importância × Evangélico',
    'relig_r_x_GrupoEv': 'Autoidentificação × Evangélico',
    'dummy_sex': 'Sexo (1=Masculino)',
    'dummy_ethnic': 'Raça/Etnia (1=Branco)',
    'escol_z': 'Escolaridade',
    'ano_ord': 'Ano Ordinal (coorte temporal)',
    'const': 'Constante'
}

def preparar_dados(df):
    """
    Prepara os dados para análise, criando interações e removendo missing.
    ...
    Returns:
    --------
    DataFrame limpo e preparado com interações
    """
    # Variáveis do modelo base
    vars_base = [
        'freq_inv_z',      # Freqi
        'pert_r_z',       # Perti
        ...
        'ano_ord'         # Controle: ano/coorte temporal
    ]
    ...
    todas_vars = vars_dependentes + vars_base
    ...
    # Criar interações com GrupoEv
    print(f"  Criando interações com GrupoEv...")
    df_clean['freq_inv_z_x_GrupoEv'] = df_clean['freq_inv_z'] * df_clean['GrupoEv']
    df_clean['pert_r_z_x_GrupoEv'] = df_clean['pert_r_z'] * df_clean['GrupoEv']
    df_clean['import_relig_z_x_GrupoEv'] = df_clean['import_relig_z'] * df_clean['GrupoEv']
    df_clean['relig_r_x_GrupoEv'] = df_clean['relig_r'] * df_clean['GrupoEv']
    
    print(f"    ✓ Frequência × Evangélico")
    print(f"    ✓ Pertencimento × Evangélico")
    print(f"    ✓ Importância × Evangélico")
    print(f"    ✓ Autoidentificação × Evangélico")
...
def criar_grafico_coeficientes(resultados_df, output_path):
    ...
    # Ordenar variáveis (manter ordem lógica: base, depois interações)
    ordem_variaveis = [
        ...
        'Evangélico',
        'Frequência a serviços religiosos × Evangélico',
        'Pertencimento × Evangélico',
        'Importância × Evangélico',
        'Autoidentificação × Evangélico',
        'Sexo (1=Masculino)',
        'Raça/Etnia (1=Branco)',
        'Escolaridade',
        'Ano Ordinal (coorte temporal)'
    ]
    ...
    ax.set_title('Modelo com Interações: Variáveis Religiosas × Evangélico', 
...
def main():
    """Função principal."""
    print("="*70)
    print("MODELO COM INTERAÇÕES: VARIÁVEIS RELIGIOSAS × EVANGÉLICO")
    print("="*70)
    print("\nModelo teórico:")
    print("Yi = β₀ + (ModeloBase) + β₁(Freqi × GrupoEvi) + β₂(Perti × GrupoEvi) + γXi + εi")
    print("\nEstimação: OLS com erros robustos de Huber-White (HC3)")
    print("="*70)
    ...
    # Preparar dados
    print(f"\n[2] Preparando dados e criando interações...")
    ...
    # Definir variáveis independentes (modelo base + interações)
    vars_independentes = [
        # Modelo base
        'freq_inv_z',      # β₁: Freqi
        'pert_r_z',       # β₂: Perti
        ...
        'GrupoEv',        # β₅: GrupoEvi
        # Interações
        'freq_inv_z_x_GrupoEv',      # β₁: Freqi × GrupoEvi
        'pert_r_z_x_GrupoEv',        # β₂: Perti × GrupoEvi
        'import_relig_z_x_GrupoEv',  # β₃: Importi × GrupoEvi
        'relig_r_x_GrupoEv',         # β₄: Identi × GrupoEvi
        # Controles
        'dummy_sex',      # γ₁: Sexo
        'dummy_ethnic',   # γ₂: Raça/Etnia
        'escol_z',        # γ₃: Escolaridade
        'ano_ord'         # γ₄: Ano/Coorte
    ]
    ...
    # Salvar resultados
    output_coef = BASE_DIR / 'resultados_modelo_interacoes_coeficientes.csv'
    output_stats = BASE_DIR / 'resultados_modelo_interacoes_estatisticas.csv'
    output_grafico = BASE_DIR / 'grafico_coeficientes_modelo_interacoes.png'
    ...
    print(f"  - Coeficientes de interação indicam se o efeito das variáveis")
    print(f"    religiosas difere entre evangélicos e não-evangélicos.")
    print(f"  - Efeito total para evangélicos = coeficiente base + coeficiente interação")
    print(f"  - Efeito total para não-evangélicos = apenas coeficiente base")
