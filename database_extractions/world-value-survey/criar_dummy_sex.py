"""
Script para criar variável dummy_sex a partir das variáveis de sexo de cada onda.
Usa leitura direta de CSV para evitar conflitos com indexes do banco.

Passo 1: Identifica as variáveis de sexo em cada onda
Passo 2: Converte valores negativos em missing
Passo 3: Recodifica em dummy_sex (1 se mantém 1, todas as outras 0)
"""
...
def process_wave(wave_dir, config):
    ...
    # Adicionar coluna dummy_sex ao header
    header.append('dummy_sex')
    ...
    if original_values:
        ...
        print(f"  Valores únicos: {sorted(freq_original.keys())}")
        ...
        if len(row) > var_index:
            ...
            try:
                if value_str:
                    ...
                    # Passo 3: Recodificar em dummy_sex
                    # Original 1 se mantém 1, todas as outras 0
                    if value is None:
                        dummy_value = ''
                        missing_count += 1
                    elif value == 1.0:
    ...
    print(f"  ✓ dummy_sex = 1 (original 1): {dummy_1_count}")
    print(f"  ✓ dummy_sex = 0 (outros): {dummy_0_count}")
    print(f"  ✓ Missing: {missing_count}")
    
    # Salvar arquivo processado
    print(f"\n[PASSO 3] Salvando arquivo processado...")
    
    output_file = BASE_DIR / wave_dir / f"{file_original.replace('.csv', '')}_com_dummy_sex.csv"
    ...
    try:
        ...
        print(f"  dummy_sex = 1: {dummy_1_count} ({dummy_1_count/len(rows)*100:.2f}%)")
        print(f"  dummy_sex = 0: {dummy_0_count} ({dummy_0_count/len(rows)*100:.2f}%)")
...
def main():
    """Função principal."""
    print("="*70)
    print("CRIAÇÃO DA VARIÁVEL dummy_sex")
    print("="*70)
    print("\nEste script processa as variáveis de sexo de cada onda:")
    print("  - Wave 2 (1991): V353")
    print("  - Wave 3 (1997): V214")
    print("  - Wave 5 (2005): V235")
    print("  - Wave 6 (2014): V240")
    print("  - Wave 7 (2018): Q260")
    print("\nRecodificação:")
    print("  - Valor original 1 → dummy_sex = 1")
    print("  - Outros valores → dummy_sex = 0")
