"""
Script para testar o modelo com efeito da percepção de AMEAÇA MORAL
e interação com grupo religioso (evangélico vs não-evangélico).

Modelo teórico:
Yi = β₀ + (ModeloBase) + β₆Ameaçai + β₁(Ameaçai × GrupoEvi) + γXi + εi
...
- Ameaçai: percepção de ameaça moral (ameaca_tipo_moral)
- Ameaçai × GrupoEvi: interação entre ameaça e ser evangélico
...
MAPEAMENTO_NOMES = {
    ...
    'ameaca_tipo_moral_x_GrupoEv': 'Ameaça moral × Evangélico',
...
def preparar_dados(df):
    """
    Prepara os dados para análise, criando interação e removendo missing.
    ...
    Returns:
    --------
    DataFrame limpo e preparado com interação
    """
    # Variáveis do modelo
    vars_modelo = [
        'freq_inv_z',      # Freqi
    ...
    # Criar interação Ameaça × GrupoEv
    print(f"  Criando interação Ameaça × Evangélico...")
    df_clean['ameaca_tipo_moral_x_GrupoEv'] = df_clean['ameaca_tipo_moral'] * df_clean['GrupoEv']
    print(f"    ✓ Ameaça moral × Evangélico")
...
def criar_grafico_coeficientes(resultados_df, output_path):
    ...
    ax.set_title('Modelo com Efeito da Ameaça Moral × Evangélico', 
...
def main():
    """Função principal."""
    print("="*70)
    print("MODELO COM EFEITO DA AMEAÇA MORAL × EVANGÉLICO")
    print("="*70)
    print("\nModelo teórico:")
    print("Yi = β₀ + (ModeloBase) + β₆Ameaçai + β₁(Ameaçai × GrupoEvi) + γXi + εi")
    print("\nEstimação: OLS com erros robustos de Huber-White (HC3)")
    print("="*70)
    ...
    # Preparar dados
    print(f"\n[2] Preparando dados e criando interação...")
    ...
    vars_independentes = [
        ...
        'GrupoEv',        # β₅: GrupoEvi
        # Efeito da ameaça
        'ameaca_tipo_moral',  # β₆: Ameaçai
        # Interação
        'ameaca_tipo_moral_x_GrupoEv',  # β₁: Ameaçai × GrupoEvi
        # Controles
        'dummy_sex',      # γ₁: Sexo
    ...
    for var_dep in vars_dependentes:
        ...
        print(f"\n  Coeficientes:")
        print(f"    {'Variável':<45s} {'Coef.':>10s} {'EP':>10s} {'t':>8s} {'p-valor':>10s} {'Sig.':>5s}")
        print(f"    {'─'*45} {'─'*10} {'─'*10} {'─'*8} {'─'*10} {'─'*5}")
        
        for _, row in resultados_df.iterrows():
            print(f"    {row['Nome_Legível']:<45s} {row['Coeficiente']:>10.4f} "
    ...
    # Salvar resultados
    output_coef = BASE_DIR / 'resultados_modelo_ameaca_moral_interacao_coeficientes.csv'
    output_stats = BASE_DIR / 'resultados_modelo_ameaca_moral_interacao_estatisticas.csv'
    output_grafico = BASE_DIR / 'grafico_coeficientes_modelo_ameaca_moral_interacao.png'
    ...
    print(f"  - O coeficiente de interação indica se o efeito da ameaça")
    print(f"    difere entre evangélicos e não-evangélicos.")
    print(f"  - Efeito total da ameaça para evangélicos = β₆ + β₁")
    print(f"  - Efeito total da ameaça para não-evangélicos = β₆")
