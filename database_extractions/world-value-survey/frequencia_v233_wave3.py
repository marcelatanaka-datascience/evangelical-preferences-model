"""
Script para calcular a frequência da variável V233 da wave 3 do World Values Survey.
...
# Encontrar o índice da coluna V233
def find_v233_index(header_row):
    """Encontra o índice da coluna V233 no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v233_index = find_v233_index(header)
    
    if v233_index is None:
        print("ERRO: Coluna V233 não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V233 encontrada no índice: {v233_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v233_index:
            value = row[v233_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v233_values.append(float(value))
                else:
                    v233_values.append(None)
            except ValueError:
                v233_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V233 (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v233_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V233 (Wave 3 - 1997)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v233_values)
...
numeric_values = [v for v in v233_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
