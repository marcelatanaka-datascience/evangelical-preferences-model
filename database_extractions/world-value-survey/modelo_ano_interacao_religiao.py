"""
Script para testar o modelo com efeito do ANO da pesquisa e interação
com grupo religioso (evangélico vs não-evangélico).

Modelo teórico:
Yi = β₀ + (ModeloBase) + β₆Ano_t + β₁(Ano_t × GrupoEvi) + γXi + εi
...
- Ano_t: ano da pesquisa (ano_ord)
- Ano_t × GrupoEvi: interação entre ano e ser evangélico
...
MAPEAMENTO_NOMES = {
    ...
    'GrupoEv': 'Evangélico',
    'ano_ord': 'Ano da pesquisa',
    'ano_ord_x_GrupoEv': 'Ano da pesquisa × Evangélico',
...
def preparar_dados(df):
    ...
    # Criar interação Ano × GrupoEv
    print(f"  Criando interação Ano × Evangélico...")
    df_clean['ano_ord_x_GrupoEv'] = df_clean['ano_ord'] * df_clean['GrupoEv']
    print(f"    ✓ Ano da pesquisa × Evangélico")
...
def criar_grafico_coeficientes(resultados_df, output_path):
    ...
    ax.set_title('Modelo com Efeito do Ano × Evangélico', 
...
def main():
    """Função principal."""
    print("="*70)
    print("MODELO COM EFEITO DO ANO × EVANGÉLICO")
    print("="*70)
    print("\nModelo teórico:")
    print("Yi = β₀ + (ModeloBase) + β₆Ano_t + β₁(Ano_t × GrupoEvi) + γXi + εi")
    print("\nEstimação: OLS com erros robustos de Huber-White (HC3)")
    print("="*70)
    ...
    vars_independentes = [
        ...
        'ano_ord',        # β₆: Ano_t
        # Interação
        'ano_ord_x_GrupoEv',  # β₁: Ano_t × GrupoEvi
        # Controles
        'dummy_sex',      # γ₁: Sexo
    ...
    # Salvar resultados
    output_coef = BASE_DIR / 'resultados_modelo_ano_interacao_coeficientes.csv'
    output_stats = BASE_DIR / 'resultados_modelo_ano_interacao_estatisticas.csv'
    output_grafico = BASE_DIR / 'grafico_coeficientes_modelo_ano_interacao.png'
    ...
    print(f"  - O coeficiente de interação indica se o efeito do ano")
    print(f"    difere entre evangélicos e não-evangélicos.")
    print(f"  - Efeito total do ano para evangélicos = β₆ + β₁")
    print(f"  - Efeito total do ano para não-evangélicos = β₆")
