"""
Script para calcular a frequência da variável Q289 da wave 7 do World Values Survey.
...
# Encontrar o índice da coluna Q289
def find_q289_index(header_row):
    """Encontra o índice da coluna Q289 no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    q289_index = find_q289_index(header)
    
    if q289_index is None:
        print("ERRO: Coluna Q289 não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna Q289 encontrada no índice: {q289_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > q289_index:
            value = row[q289_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    q289_values.append(float(value))
                else:
                    q289_values.append(None)
            except ValueError:
                q289_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna Q289 (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(q289_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL Q289 (Wave 7 - 2018)")
print("=" * 60)

# Contar frequências
frequencies = Counter(q289_values)
...
numeric_values = [v for v in q289_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
