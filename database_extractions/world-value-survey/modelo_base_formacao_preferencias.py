"""
Script para testar o modelo base de formação de preferências políticas
segundo a teoria proposta na tese.

Modelo teórico:
Yi = β₀ + β₁Freqi + β₂Perti + β₃Importi + β₄Identi + β₅GrupoEvi + γXi + εi
...
def criar_grafico_coeficientes(resultados_df, output_path):
    ...
    resultados_df : DataFrame
        DataFrame com resultados dos modelos (deve ter colunas: Variável_Dependente,
        Nome_Legível, Coeficiente, IC_95_inf, IC_95_sup, p_valor)
    output_path : Path
    ...
    ax.axvline(x=0, color='black', linestyle='--', linewidth=1, alpha=0.5, zorder=0)
    ...
    ax.set_title('Coeficientes do Modelo Base de Formação de Preferências Políticas', 
    ...
    # Adicionar linhas de referência em valores importantes
...
def main():
    """Função principal."""
    print("="*70)
    print("MODELO BASE DE FORMAÇÃO DE PREFERÊNCIAS POLÍTICAS")
    print("="*70)
    print("\nModelo teórico:")
    print("Yi = β₀ + β₁Freqi + β₂Perti + β₃Importi + β₄Identi + β₅GrupoEvi + γXi + εi")
    print("\nEstimação: OLS com erros robustos de Huber-White (HC3)")
    print("="*70)
    ...
    for var_dep in vars_dependentes:
        ...
        print(f"\n  Coeficientes:")
        print(f"    {'Variável':<30s} {'Coef.':>10s} {'EP':>10s} {'t':>8s} {'p-valor':>10s} {'Sig.':>5s}")
        print(f"    {'─'*30} {'─'*10} {'─'*10} {'─'*8} {'─'*10} {'─'*5}")
        
        for _, row in resultados_df.iterrows():
            print(f"    {row['Nome_Legível']:<30s} {row['Coeficiente']:>10.4f} "
    ...
    # Salvar resultados
    output_coef = BASE_DIR / 'resultados_modelo_base_coeficientes.csv'
    output_stats = BASE_DIR / 'resultados_modelo_base_estatisticas.csv'
    output_grafico = BASE_DIR / 'grafico_coeficientes_modelo_base.png'
