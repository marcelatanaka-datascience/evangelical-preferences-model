"""
Script para criar as variáveis Freq_inv e Freq_inv_z (frequência a serviços religiosos).
Passos:
1. Usar arquivos originais de cada onda
2. Converter todos os valores negativos em missing
3. Inverter os valores originais e criar coluna Freq_inv
4. Padronizar Freq_inv em z-score e criar coluna Freq_inv_z
"""
...
WAVES_CONFIG = {
    'wave-2-1991-br': {
        ...
        'file_output': 'WV2_Data_Brazil_Csv_v1.6.1_com_freq_inv.csv',
        'year': 1991,
        'variable': 'V147',
    ...
    'wave-3-1997-br': {
        ...
        'file_output': 'WV3_Data_Brazil_Csv_v20221107.1_com_freq_inv.csv',
        'year': 1997,
        'variable': 'V181',
    ...
    'wave-5-2006-br': {
        ...
        'file_output': 'WV5_Data_Brazil_Csv_v20201117.1_com_freq_inv.csv',
        'year': 2005,
        'variable': 'V186',
        'scale_max': 7  # Escala 1-7
    },
    'wave-6-2014-br': {
        ...
        'file_output': 'WV6_Data_Brazil_Csv_v20221117.1_com_freq_inv.csv',
        'year': 2014,
        'variable': 'V145',
        'scale_max': 7  # Escala 1-7
    },
    'wave-7-2018-br': {
        ...
        'file_output': 'WVS_Wave_7_Brazil_Csv_v5.1_com_freq_inv.csv',
        'year': 2018,
        'variable': 'Q171',
        'scale_max': 7  # Escala 1-7
    }
...
def process_variable(df, var_col, scale_max):
    """
    Processa a variável seguindo os passos:
    1. Converte valores negativos em missing
    2. Inverte os valores (cria Freq_inv)
    3. Padroniza em z-score (cria Freq_inv_z)
    """
    # Passo 2: Converter valores negativos em missing
    df[var_col + '_processed'] = df[var_col].copy()
    df.loc[df[var_col] < 0, var_col + '_processed'] = np.nan
    
    # Passo 3: Inverter valores
    # Identifica valores válidos (>= 1 e <= scale_max)
    valid_mask = (df[var_col + '_processed'] >= 1) & (df[var_col + '_processed'] <= scale_max)
    
    # Inverte: novo_valor = (scale_max + 1) - valor_original
    df['Freq_inv'] = np.nan
    df.loc[valid_mask, 'Freq_inv'] = (scale_max + 1) - df.loc[valid_mask, var_col + '_processed']
    
    # Passo 4: Padronizar em z-score
    valid_values = df['Freq_inv'].dropna()
    
    if len(valid_values) == 0:
        df['Freq_inv_z'] = np.nan
        return False
    ...
    df['Freq_inv_z'] = (df['Freq_inv'] - mean) / std
    
    # Remove coluna temporária
    df.drop(columns=[var_col + '_processed'], inplace=True)
...
def process_wave(wave_name, config):
    ...
    # Passo 3: Inverter valores e criar Freq_inv
    print(f"\n[PASSO 3] Invertendo valores e criando Freq_inv...")
    valid_mask = (df[variable + '_temp'] >= 1) & (df[variable + '_temp'] <= scale_max)
    
    df['Freq_inv'] = np.nan
    df.loc[valid_mask, 'Freq_inv'] = (scale_max + 1) - df.loc[valid_mask, variable + '_temp']
    
    # Estatísticas
    original_valid = df[variable + '_temp'].dropna()
    inverted_valid = df['Freq_inv'].dropna()
    
    print(f"  ✓ {variable} -> Freq_inv")
    ...
    # Passo 4: Padronizar em z-score e criar Freq_inv_z
    print(f"\n[PASSO 4] Padronizando Freq_inv em z-score e criando Freq_inv_z...")
    valid_values = df['Freq_inv'].dropna()
    ...
    if len(valid_values) == 0:
        ...
        if std == 0:
            ...
            valid_z = df['Freq_inv_z'].dropna()
            print(f"  ✓ Freq_inv -> Freq_inv_z")
            print(f"    Média: {df['Freq_inv_z'].mean():.6f}")
            print(f"    DP: {df['Freq_inv_z'].std():.6f}")
    ...
    try:
        ...
        print(f"  ✓ Novas colunas criadas: Freq_inv, Freq_inv_z")
    except Exception as e:
    ...
    result_df['Freq_inv'] = df['Freq_inv']
    result_df['Freq_inv_z'] = df['Freq_inv_z']
...
def combine_all_waves(combined_results):
    ...
    print(f"\n{'─'*70}")
    print("ESTATÍSTICAS DESCRITIVAS DE Freq_inv:")
    print(f"{'─'*70}\n")
    
    valid = combined_df['Freq_inv'].dropna()
    ...
    print(f"\n{'─'*70}")
    print("ESTATÍSTICAS DESCRITIVAS DE Freq_inv_z:")
    print(f"{'─'*70}\n")
    
    valid_z = combined_df['Freq_inv_z'].dropna()
    ...
    print(f"\n{'─'*70}")
    print("ESTATÍSTICAS DE Freq_inv POR ONDA:")
    print(f"{'─'*70}\n")
    
    stats_by_wave = combined_df.groupby('wave')['Freq_inv'].agg([
    ...
    print(f"\n{'─'*70}")
    print("ESTATÍSTICAS DE Freq_inv_z POR ONDA:")
    print(f"{'─'*70}\n")
    
    stats_by_wave_z = combined_df.groupby('wave')['Freq_inv_z'].agg([
        'count', 'mean', 'std', 'min', 'max'
    ]).round(4)
    ...
    output_file = BASE_DIR / 'freq_inv_empilhado_completo.csv'
...
def main():
    """Função principal."""
    print("="*70)
    print("CRIAÇÃO DAS VARIÁVEIS Freq_inv E Freq_inv_z")
    ...
    if combined_results:
        ...
        if combined_df is not None:
            ...
            print(f"Variáveis Freq_inv e Freq_inv_z criadas para todas as ondas!")
