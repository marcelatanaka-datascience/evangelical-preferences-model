"""
Script para calcular a frequência da variável V185G da wave 5 do World Values Survey.
...
CSV_FILE = BASE_DIR / 'wave-5-2006-br' / 'WV5_Data_Brazil_Csv_v20201117.1.csv'

# Encontrar o índice da coluna V185G
def find_v185g_index(header_row):
    """Encontra o índice da coluna V185G no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v185g_index = find_v185g_index(header)
    
    if v185g_index is None:
        print("ERRO: Coluna V185G não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V185G encontrada no índice: {v185g_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v185g_index:
            value = row[v185g_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v185g_values.append(float(value))
                else:
                    v185g_values.append(None)
            except ValueError:
                v185g_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V185G (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v185g_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V185G (Wave 5 - 2006)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v185g_values)
...
numeric_values = [v for v in v185g_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
