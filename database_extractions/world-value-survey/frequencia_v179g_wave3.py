"""
Script para calcular a frequência da variável V179G da wave 3 do World Values Survey.
...
CSV_FILE = BASE_DIR / 'wave-3-1997-br' / 'WV3_Data_Brazil_Csv_v20221107.1.csv'

# Encontrar o índice da coluna V179G
def find_v179g_index(header_row):
    """Encontra o índice da coluna V179G no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v179g_index = find_v179g_index(header)
    
    if v179g_index is None:
        print("ERRO: Coluna V179G não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V179G encontrada no índice: {v179g_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v179g_index:
            value = row[v179g_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v179g_values.append(float(value))
                else:
                    v179g_values.append(None)
            except ValueError:
                v179g_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V179G (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v179g_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V179G (Wave 3 - 1997)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v179g_values)
...
numeric_values = [v for v in v179g_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
