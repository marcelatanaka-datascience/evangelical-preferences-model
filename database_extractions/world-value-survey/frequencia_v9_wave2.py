"""
Script para calcular a frequência da variável V9 da wave 2 do World Values Survey.
Usa leitura manual de CSV.
"""
...
# Encontrar o índice da coluna V9
def find_v9_index(header_row):
    """Encontra o índice da coluna V9 no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v9_index = find_v9_index(header)
    
    if v9_index is None:
        print("ERRO: Coluna V9 não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V9 encontrada no índice: {v9_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v9_index:
            value = row[v9_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v9_values.append(float(value))
                else:
                    v9_values.append(None)
            except ValueError:
                v9_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V9 (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v9_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V9 (Wave 2 - 1991)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v9_values)
...
numeric_values = [v for v in v9_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
