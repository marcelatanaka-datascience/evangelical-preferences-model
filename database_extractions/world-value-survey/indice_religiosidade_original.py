...
VERSÃO SEM NORMALIZAÇÃO HARDCODED - USA VALORES ORIGINAIS
...
def clean_value(value):
    """
    Limpa valores, removendo apenas códigos de missing do WVS.
    Retorna o valor original se válido, np.nan se missing.
    
    No WVS, valores negativos geralmente indicam missing.
    """
    if pd.isna(value):
        return np.nan
    ...
    try:
        val = float(value)
        # Valores negativos são códigos de missing no WVS
        if val < 0:
            return np.nan
        return val
    except:
...
def process_wave(wave_name, wave_config):
    """Processa uma onda específica e retorna DataFrame com valores originais (sem normalização)."""
    ...
    # Processa cada dimensão usando valores originais (apenas limpa missing)
    results = {}
    
    # 1. Saliencia Religiosa
    if wave_config['saliencia'] in df.columns:
        results['saliencia'] = df[wave_config['saliencia']].apply(clean_value)
    else:
    ...
    if wave_config['religiosidade_individual'] in df.columns:
        results['religiosidade_individual'] = df[wave_config['religiosidade_individual']].apply(clean_value)
    else:
    ...
    if wave_config['frequencia'] in df.columns:
        results['frequencia'] = df[wave_config['frequencia']].apply(clean_value)
    else:
    ...
    if wave_config['organizacao'] in df.columns:
        results['organizacao'] = df[wave_config['organizacao']].apply(clean_value)
    else:
...
def calculate_cronbach_alpha(combined_df):
    ...
    corr_file_pearson = BASE_DIR / 'correlacao_pearson_dimensoes_original.csv'
    corr_file_spearman = BASE_DIR / 'correlacao_spearman_dimensoes_original.csv'
    corr_file_kendall = BASE_DIR / 'correlacao_kendall_dimensoes_original.csv'
    ...
    cronbach_file = BASE_DIR / 'cronbach_alpha_indice_religiosidade_original.csv'
...
def calculate_ordinal_measures(combined_df):
    ...
    ordinal_file = BASE_DIR / 'ordinal_alpha_indice_religiosidade_original.csv'
    ordinal_df.to_csv(ordinal_file, index=False, encoding='utf-8')
    print(f"\nResultados do Alfa Ordinal salvos em: {ordinal_file}")
    
    polychoric_file = BASE_DIR / 'correlacao_polychoric_dimensoes_original.csv'
...
def test_multicollinearity(combined_df):
    ...
    multicollinearity_file = BASE_DIR / 'multicolinearidade_indice_religiosidade_original.csv'
...
def main():
    ...
    print("VERSÃO SEM NORMALIZAÇÃO HARDCODED - VALORES ORIGINAIS")
    print("=" * 60)
    ...
    # Salva resultados
    output_file = BASE_DIR / 'indice_religiosidade_completo_original.csv'
    combined_df.to_csv(output_file, index=False, encoding='utf-8')
    print(f"\nResultados salvos em: {output_file}")
    
    # Salva estatísticas por onda
    stats_file = BASE_DIR / 'estatisticas_indice_religiosidade_original.csv'
