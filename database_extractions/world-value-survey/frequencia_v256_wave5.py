"""
Script para calcular a frequência da variável V256 da wave 5 do World Values Survey.
...
# Encontrar o índice da coluna V256
def find_v256_index(header_row):
    """Encontra o índice da coluna V256 no cabeçalho."""
...
with open(CSV_FILE, 'r', encoding='utf-8') as f:
    ...
    # Ler o cabeçalho
    header = next(reader)
    v256_index = find_v256_index(header)
    
    if v256_index is None:
        print("ERRO: Coluna V256 não encontrada no arquivo!")
        print("Colunas disponíveis:", header[:20], "...")
        exit(1)
    
    print(f"Coluna V256 encontrada no índice: {v256_index}")
    ...
    for row_num, row in enumerate(reader, start=2):  # start=2 porque já lemos o header
        if len(row) > v256_index:
            value = row[v256_index].strip('"').strip()
            # Converter para float se possível, senão manter como string
            try:
                if value:
                    v256_values.append(float(value))
                else:
                    v256_values.append(None)
            except ValueError:
                v256_values.append(value)
        else:
            print(f"Aviso: Linha {row_num} não tem coluna V256 (apenas {len(row)} colunas)")

print(f"Total de registros lidos: {len(v256_values)}")
print("-" * 60)

# Calcular frequências
print("\nFREQUÊNCIA DA VARIÁVEL V256 (Wave 5 - 2006)")
print("=" * 60)

# Contar frequências
frequencies = Counter(v256_values)
...
numeric_values = [v for v in v256_values if v is not None and isinstance(v, (int, float)) and v >= 0]
if numeric_values:
    print("\n" + "=" * 60)
